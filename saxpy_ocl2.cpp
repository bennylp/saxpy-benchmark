...
int main(int argc, char *argv[]) {
      ...
      if (CL_SUCCESS == status) {
         ...
         if (CL_SUCCESS == status) {
            ...
            for (size_t j = 0; j < numDevices; j++) {
               ...
               status = clGetDeviceInfo(deviceIDs[j], CL_DEVICE_TYPE,
   ...
   cl_mem memX = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
                                N * sizeof(float), cpuX, &status);
   exitOnFail(status, "create buffer for X");

   // third argument: memory buffer object for Y
   cl_mem memY = clCreateBuffer(context,
                                CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(float), cpuY,
                                &status);
   ...
   status = clEnqueueWriteBuffer(queue, memX, CL_FALSE, 0, N * sizeof(float),
                                 cpuX, 0, NULL, &event);
   ...
   status = clEnqueueWriteBuffer(queue, memY, CL_FALSE, 0, N * sizeof(float),
                                 cpuY, 0, NULL, &event);
   ...
   cl_program program = clCreateProgramWithSource(context,
                           sizeof(kernelSrc) / sizeof(const char*), kernelSrc, NULL, &status);
   ...
   status = clEnqueueNDRangeKernel(queue, kernel,
                                    sizeof(global) / sizeof(size_t), NULL,
   ...
   // data transfer for array Y
   status = clEnqueueReadBuffer(queue, memY, CL_FALSE, 0, N * sizeof(float),
                                cpuY, 0, NULL, &event);
   ...
   std::cout << "Total time: " << timer_total.elapsed_msec() << " ms\n";
